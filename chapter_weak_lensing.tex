...
Using the definition of  convergence as a function of the matter density contrast given at \autoref{eq:kappadeltam} and assuming a constant redshift-independent galaxy-bias, $\delta_g(\hat n,z_L) = b_L\delta_M(\hat n,z_L)$, this leads finally to
\begin{equation}
\omega_{LS}(\theta) = b_L[\alpha(m)-1]2\kappa(\theta),
\label{eq:numbercountstheo}
\end{equation}
where $b_L$ is the galaxy-bias of the lens sample and it has been used that $\kappa(\theta)=\langle\delta_g(\hat n,z_L)\kappa(\hat n',z_S)\rangle_\theta$. That is, the convergence profile is the average convergence field at an angular distance $\theta$ around the selected lenses. It can be expressed as
\begin{eqnarray}
&\kappa(\theta)=\frac{3H_0^2\Omega_M^0}{c^2}\int\limits_0^\infty dz'_L\frac{\phi_L(z'_L)}{1+z'_L}\int\limits_i^\infty dz'_S\phi_S(z'_S)\frac{r(z'_L)[r(z'_S)-r(z'_L)]}{r(z'_S)}\times\\
...
\end{eqnarray}
Here $r(z)$ is the radial comoving at redshift $z$, whereas $z_v$ is the redshit of the center of the void and $\theta$ the angular separation form the center of the void. From \autoref{eq:kgb} and \autoref{eq:kappa_definition}, the convergence profile around a single void can be obtained:
\begin{equation}
