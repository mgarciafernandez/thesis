...
The goal of this analysis is to use the methodology to measure weak-lensing magnification signal developed at \autoref{sec:analysis_sv} to determine the convergence profile of voids and troughs with the DES-Y1 data. Data sample is described at \autoref{sec:data_sample_y1}. Then, the analysis is described \autoref{sec:analysis_y1}. Finally a discussion on the analysis can be found at \autoref{sec:discussion_y1}.
\newline

Voids and troughs weak-lensing analyses has already been made with the DES-SV data \cite{2016MNRAS.455.3367G,2017MNRAS.465..746S} using gg-lensing. Nevertheless, the small area of the Science Verification data-set does not allow to detect the magnification signal of these lenses.
...
On this section, the convergence profile of the empties regions of the Universe --voids and troughs-- has been computed. For the first time,a negative convergence profile has been measured using number count magnification. The measured magnification signal has been detected for both voids an troughs, being less significant the detection of voids since there is a small number of them.
\newline
...
This analysis demonstrates that number-count magnification by voids and troughs constitutes a new, independent and competitive probe for cosmology. On the near future, when the DES Year 5 data will be available, the size of the errors will be reduced by a factor 2, providing cosmological results with unprecedented accuracy on weak-lensing magnification.
